...
import { getImageUrl } from '../../services/api'; // ✅ Utilisation de getImageUrl

const RapportsSection = () => { // ✅ Nom du composant changé
    const [rapports, setRapports] = useState([]); // ✅ Variable renommée
    ...
    useEffect(() => {
        const fetchRapports = async () => {
            try {
                ...
                setRapports(rapportsData.slice(0, 3));
            } catch (error) {
    ...
    const handleDownload = async (e, fileUrl, fileName) => {
        ...
        e.stopPropagation();
        if (!fileUrl) return;
        const fullUrl = fileUrl.startsWith('http') ? fileUrl : getImageUrl(fileUrl); // ✅ getImageUrl utilisé
        try {
        ...
        <section data-theme="light" className="py-16 sm:py-24 bg-gradient-to-br from-slate-50 via-white to-cyan-50/30">
            <div className="container">
                ...
                {loading ? (
                    ...
                    <>
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mt-12">
                            {rapports.map((rapport, index) => { // ✅ Variable renommée
                                return (
                                    <Link
                                        to={`/rapports/${rapport.slug}`} // ✅ URL changée
                                        key={rapport._id}
                                    ...
                                    >
                                        {/* Image de couverture avec getImageUrl */}
                                        <img
                                            src={rapport.coverImage ? getImageUrl(rapport.coverImage) : '/placeholder.jpg'} // ✅ getImageUrl utilisé
                                            alt={rapport.title}
                                            loading="lazy"
                                        ...
                                        <div className="absolute inset-0 p-6 flex flex-col justify-between">
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex items-center justify-between pt-2">
                                                    ...
                                                    <button 
                                                        onClick={(e) => handleDownload(e, rapport.fileUrl, `${rapport.title}.pdf`)}
