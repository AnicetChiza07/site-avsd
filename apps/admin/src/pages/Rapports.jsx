// ===========================================
// PAGE GESTION DES RAPPORTS - VERSION COMPACTE
// ===========================================
...
import { getImageUrl } from '../services/api'; // Utilisation de la fonction robuste
...
const Rapports = () => {
    ...
    const [deleteModal, setDeleteModal] = useState({ open: false, rapport: null });
    ...
    useEffect(() => {
        const fetchRapports = async () => {
            try {
                ...
                const years = [...new Set(data.map(rapport => 
                    new Date(rapport.publishedAt || rapport.createdAt).getFullYear()
                ))].sort((a, b) => b - a);
    ...
    const openEditModal = (rapport) => {
        ...
        setFormData({
            ...
            featured: rapport.featured
        });
        setCoverImageFile(null);
        // Utilisation de getImageUrl pour une gestion robuste
        setCoverImagePreview(rapport.coverImage ? getImageUrl(rapport.coverImage) : null);
        setPdfFile(null);
        setExistingPdfName(rapport.fileUrl ? 'PDF actuel' : null);
        setIsFormModalOpen(true);
    };
    ...
    const handleSubmit = async (e) => {
        ...
        if (!editingRapport && !coverImageFile) errors.push('L\'image de couverture est obligatoire');
        if (!editingRapport && !pdfFile) errors.push('Le PDF est obligatoire');
        ...
        try {
            ...
            if (editingRapport) {
                await archiveService.updateArchive(editingRapport._id, data);
            ...
            setRapports(dataRes);
            const years = [...new Set(dataRes.map(rapport => 
                new Date(rapport.publishedAt || rapport.createdAt).getFullYear()
            ))].sort((a, b) => b - a);
            setAvailableYears(years);
    ...
    const handleDelete = async () => {
        if (!deleteModal.rapport) return;
        try {
            await archiveService.deleteArchive(deleteModal.rapport._id);
            ...
            setDeleteModal({ open: false, rapport: null });
        } catch (err) {
    ...
    // Filtrer les rapports par année
    const filteredRapports = yearFilter 
        ? rapports.filter(rapport => new Date(rapport.publishedAt || rapport.createdAt).getFullYear().toString() === yearFilter)
        : rapports;
    ...
    return (
        <AdminLayout>
            ...
            {/* Liste des rapports - TABLEAU COMPACT */}
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
                {filteredRapports.length === 0 ? (
                    <div className="text-center py-16">
                        <FileText className="w-12 h-12 text-gray-300 mx-auto mb-4" />
                        <h3 className="text-lg font-semibold text-gray-900 mb-2">
                            {yearFilter ? `Aucun rapport en ${yearFilter}` : 'Aucun rapport'}
                        </h3>
                        <p className="text-gray-500 text-sm mb-4">
                            {yearFilter ? 'Essayez une autre année ou créez un nouveau rapport' : 'Commencez par créer votre premier rapport'}
                        </p>
                        {!yearFilter && (
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full">
                            <thead className="bg-gray-50 border-b border-gray-200">
                                <tr>
                                    <th className="text-left px-4 py-2.5 text-[13px] font-bold text-gray-500 uppercase tracking-wider">Rapport</th>
                            ...
                            <tbody className="divide-y divide-gray-100">
                                {filteredRapports.map((rapport) => (
                                    <tr key={rapport._id} className="hover:bg-gray-50/50 transition-colors group h-16">
                                            ...
                                            <div className="flex items-center gap-3 h-full">
                                                <div className="w-16 h-16 rounded-lg overflow-hidden bg-gray-100 flex-shrink-0 border border-gray-200">
                                                    <img 
                                                        ...
                                                        onError={(e) => {
                                                            ...
                                                            e.target.src = 'https://via.placeholder.com/100x100?text=Doc';
                                                        }}
                                                    />
                                                </div>
                                                <div className="min-w-0 flex-1">
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {rapport.featured && <Star className="w-3.5 h-3.5 text-yellow-500 fill-yellow-500 flex-shrink-0" />}
                                                    </div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                <div className="w-8 h-8 bg-brand-blue rounded-full flex items-center justify-center flex-shrink-0">
                                                    <span className="text-white text-xs font-bold">
                                                        {rapport.author?.initials || 'AVSD'}
                                                    </span>
                                                </div>
                                                <span className="text-[13px] text-gray-700 font-medium">
                                                    {rapport.author?.name || 'AVSD RDC'}
                                                </span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-4 py-2">
                                            <div className="flex items-center gap-1.5 text-[12px] text-gray-500 whitespace-nowrap">
                                                ...
                                                <span>
                                                    {formatDateTime(rapport.publishedAt || rapport.createdAt)}
                                                </span>
                                            </div>
                                        </td>
                                        ...
                                        <td className="px-4 py-2">
                                            <div className="flex items-center justify-end gap-1">
                                                ...
                                                <button 
                                                    onClick={() => setDeleteModal({ open: true, rapport })} 
                            ...
                            <div className="flex-shrink-0 bg-white border-b border-gray-200 px-6 py-4 flex items-center justify-between">
                                <div>
                                    ...
                                    <p className="text-gray-600 text-xs mt-0.5">
                                        {editingRapport ? 'Modifiez les informations' : 'Créez un nouveau rapport ou document'}
                                    </p>
                                </div>
                                            ...
                                            <div className="flex flex-col gap-2">
                                                ...
                                                >
                                                    {saving ? (
                                                        ...
                                                        <><Plus className="w-4 h-4" /> {editingRapport ? 'Modifier' : 'Créer'}</>
                                                    )}
                                                </button>
                                                <button
                                                    type="button"
            ...
            {deleteModal.open && (
                <>
                    <div 
                        className="fixed inset-0 bg-black/60 backdrop-blur-sm z-[60]" 
                        onClick={() => setDeleteModal({ open: false, rapport: null })} 
                    />
                    ...
                    <div className="fixed inset-0 z-[70] flex items-center justify-center p-4">
                        <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-md p-8">
                            ...
                            <h3 className="text-xl font-bold text-gray-900 text-center mb-3">Supprimer ce rapport ?</h3>
                            <p className="text-gray-600 text-center mb-8">
                                Le rapport <strong>"{deleteModal.rapport?.title}"</strong> et son PDF seront définitivement supprimés. Cette action est irréversible.
                            </p>
                            <div className="flex gap-3">
                                <button onClick={() => setDeleteModal({ open: false, rapport: null })} className="flex-1 px-6 py-3 text-gray-700 bg-gray-100 hover:bg-gray-200 font-semibold rounded-xl transition-colors">
                                    Annuler
                                </button>
